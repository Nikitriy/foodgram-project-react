...
from users.serializers import CustomUserSerializer, CustomUserCreateSerializer, SetPasswordSerializer, SubscriptionSerializer
...
def set_password(request):
    if request.method == 'POST':
        ...
        if request.user.password != serializer.data.get('current_password'):
            return Response({'error': 'Неверно введён существующий пароль'}, status=status.HTTP_400_BAD_REQUEST)
        CustomUser.objects.filter(username=request.user.username).update(password=serializer.data.get('new_password'))
